        ...
        return `
            <div id="seer-ui" class="text-center animate-fade-in bg-slate-900/60 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl backdrop-blur-md relative overflow-hidden">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ${targets.map(p => 
                        `<button onclick="window.RoleRegistry.Seer.reveal('${p.id}')" class="btn bg-black/40 backdrop-blur-md border border-white/10 text-white/90    transition-all shadow-[inset_0_0_20px_rgba(255,255,255,0.02)]  py-4 rounded-xl font-bold uppercase tracking-wider text-sm group relative overflow-hidden  active:scale-95 transition-transform ">${p.name}</button>`
                    ).join('')}
                </div>
            </div>

            <div id="seer-result" class="hidden text-center animate-fade-in py-10 bg-slate-900/60 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl backdrop-blur-md relative overflow-hidden">
