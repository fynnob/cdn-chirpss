window.RoleRegistry.Necromancer = {
    renderUI: (context, me) => {
        if(me.hasRevived) {
             return `<div class="text-center"><h2 class="text-gray-500">Power used.</h2><button class="btn btn-action mt-4 active:scale-95 transition-transform " onclick="finishTurn()">Sleep</button></div>`;
        }
        ...
        if(deadWolves.length === 0) {
             return `<div class="text-center"><h2 class="text-gray-400">No dead wolves to revive.</h2><button class="btn btn-action mt-4 active:scale-95 transition-transform " onclick="finishTurn()">Sleep</button></div>`;
        }
        ...
        return `
            <div class="text-center animate-fade-in bg-slate-900/60 border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl backdrop-blur-md relative overflow-hidden">
                ...
                <div class="grid grid-cols-2 gap-2">
                    ${deadWolves.map(p => 
                        `<button onclick="window.RoleRegistry.Necromancer.revive('${p.id}')" class="btn bg-green-900/40 border-green-500/30 active:scale-95 transition-transform ">${p.name}</button>`
                    ).join('')}
                </div>
            </div>`;
    },
